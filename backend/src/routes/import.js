...
router.get('/template.csv', async (req, res) => {
  try {
    ...
    try {
      const result = await client.query('SELECT key FROM columns_registry ORDER BY display_order, key');
      const columns = result.rows.map(row => row.key);
      
      const headers = ['report_date', 'month_label', ...columns];
      const sampleRow = [new Date().toISOString().split('T')[0], '', ...columns.map(() => '')];
