...
const CampaignSetup: React.FC = () => {
  ...
  const loadSavedCampaigns = async () => {
    ...
    try {
      const campaigns = await campaignsApi.getCampaigns({ clientId: selectedClient.id });
  ...
  const saveCampaignToBackend = async () => {
    ...
    try {
      ...
      const campaignData = {
        name: campaignName,
        type: 'brand-awareness' as const,
        budget: currentSetup.totalBudget,
        product: 'Campaign from Media Plan Setup',
        ...
        managerId: user._id,
        managerName: user.name || 'Account Manager',
